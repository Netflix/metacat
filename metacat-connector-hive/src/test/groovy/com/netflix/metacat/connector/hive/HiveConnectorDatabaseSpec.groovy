...
class HiveConnectorDatabaseSpec extends Specification {
    ...
    def "Test for create database with Exceptions"() {
        ...
        then:
        1 * client.createDatabase(new Database("testdb2", "testdb2", "file://temp/", Collections.EMPTY_MAP)) >> {
            throw exception
        }
        thrown result
    ...
    def "Test for get database not found"() {
        when:
        hiveConnectorDatabaseService.get(connectorContext, QualifiedName.ofDatabase("testhive", "testdb"))
        then:
    ...
    def "Test for get database"() {
        ...
        dbInfo.name.databaseName == dbName
        1 * localHiveClient.getDatabase("testdb") >> Mock(Database)
    }
