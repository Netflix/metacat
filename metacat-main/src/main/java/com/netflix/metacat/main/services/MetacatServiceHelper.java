...
public class MetacatServiceHelper {
    ...
    public void postPreUpdateEvent(QualifiedName name, BaseDto dto, MetacatContext metacatContext) {
        if( name.isPartitionDefinition()){
            PartitionDto partitionDto = (PartitionDto) dto;
            eventBus.post(new MetacatSaveTablePartitionPreEvent(name, Lists.newArrayList(partitionDto), metacatContext));
        } else if( name.isTableDefinition()){
    ...
    public void postPostUpdateEvent(QualifiedName name, BaseDto dto, MetacatContext metacatContext) {
        if( name.isPartitionDefinition()){
            PartitionDto partitionDto = (PartitionDto) dto;
            eventBus.post(new MetacatSaveTablePartitionPostEvent(name, Lists.newArrayList(partitionDto), metacatContext));
        } else if( name.isTableDefinition()){
            eventBus.post(new MetacatUpdateTablePostEvent((TableDto)dto, metacatContext));
        } else if( name.isDatabaseDefinition()){
