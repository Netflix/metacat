...
public class MySqlParentChildRelMetaDataService implements ParentChildRelMetadataService {
    ...
    public Pair<Set<String>, Set<String>> renameSoftInsert(final QualifiedName oldName, final QualifiedName newName) {
        ...
        final Set<String> parentUuids = renameSoftInsertParent(oldName, newName);
        final Set<String> childUuids = renameSoftInsertChild(oldName, newName);
    ...
    private Set<String> renameSoftInsertParent(final QualifiedName oldName, final QualifiedName newName) {
        try {
            jdbcTemplate.update(connection -> {
    ...
    private Set<String> renameSoftInsertChild(final QualifiedName oldName, final QualifiedName newName) {
        try {
            jdbcTemplate.update(connection -> {
    ...
    // Note this api is used for 2 cases in TableServiceImpl
    // 1. dropTable -> it will pass an empty optional, which will delete every records associated with the name
    // 2. renameTable -> it will pass a non-empty optional, which will delete every records associated with the
    // name + the uuids
    @Override
